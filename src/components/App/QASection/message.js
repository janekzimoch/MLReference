...
export default function Message(props) {
  ...
  return (
    <>
      <div className={`chat-message ${props.is_last_message ? "pb-3" : "pb-1"}`}>
        <div className={`flex items-end ${justify_style}`}>
          <div className={`${order_msg} mx-2 flex max-w-xs flex-col space-y-2 text-xs`}>
            {is_chat ? (
              <div>
                <span
          ...
          {props.is_last_message ? (
            ...
            <Image alt={""} src={pdfgpt} className={`${order_icon}  h-6 w-6 opacity-0`} />
          )}
        </div>
      </div>
    </>
